...
const DailyRecommendation = () => {
    ...
    if (loading && !libro) return (
        <div className="py-20 bg-[#f4f1ea] text-center px-4">
        ...
        <section className="py-12 md:py-16 bg-[#f4f1ea] px-4">
            <div className="max-w-5xl mx-auto">
                <div className="text-center mb-8">
                    <h2 className="text-xl md:text-2xl font-serif font-bold text-stone-800 flex items-center justify-center gap-2 uppercase tracking-widest">
                        <Sparkles className="w-5 h-5 text-amber-500" />
                        Descubrimiento del Momento
                    </h2>
                </div>

                <div className={`bg-[#fdfcf8] border border-amber-200 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl transition-all duration-300 flex flex-col md:flex-row items-stretch ${loading ? 'opacity-50 scale-[0.98]' : 'opacity-100 scale-100'}`}>
                    <div className="md:w-1/3 relative group min-h-[300px] md:min-h-[350px] bg-stone-200">
                        <img 
                        ...
                        />
                        <div className="absolute top-4 left-4 bg-amber-600 text-white px-3 py-1 rounded-full text-[9px] font-bold flex items-center gap-1 shadow-lg tracking-widest uppercase">
                            Recomendación
                        </div>
                    </div>
                    ...
                    <div className="md:w-2/3 p-6 md:p-12 flex flex-col justify-center">
                        ...
                        <h3 className="text-2xl md:text-4xl font-serif font-bold text-stone-900 mb-2 italic tracking-tight break-words">
                            {libro.book_title}
                        </h3>
                        <p className="text-amber-800 font-medium mb-4 font-sans uppercase tracking-[0.2em] text-[9px] md:text-[10px]">
                            Por {libro.author || 'Autor desconocido'} • {libro.categoria_ia || 'Literatura'}
                        </p>
                        <p className="text-stone-700 leading-relaxed mb-6 md:mb-8 italic text-base md:text-lg line-clamp-4 md:line-clamp-3">
                            "{libro.review_text}"
                        </p>
                        <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
                            <Link to={`/review/${libro.id}`} className="bg-stone-900 text-white px-8 py-3 rounded-full font-bold text-sm hover:bg-amber-700 transition-all flex items-center justify-center gap-2 shadow-lg">
                                <BookOpen className="w-4 h-4" /> Ver Crónica
                            </Link>
                            <button 
                                onClick={(e) => { e.preventDefault(); obtenerLibroAleatorio(); }} 
                                className="border border-stone-300 text-stone-600 px-6 py-3 rounded-full font-bold text-sm hover:bg-white transition-all flex items-center justify-center gap-2 active:scale-95"
                            >
...
const UserActionSection = () => (
    <section id="acciones-usuario" className="py-16 md:py-24 bg-stone-50 border-y border-stone-100 px-4">
        <div className="max-w-7xl mx-auto text-center">
            <h2 className="text-3xl md:text-4xl font-serif font-bold text-stone-800 mb-10 md:mb-12 italic">¡ Bienvenido de Nuevo !</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 md:gap-8">
                {[
                    ...
                    <Link key={idx} to={item.to} className="group p-8 md:p-10 bg-[#fdfcf8] rounded-[2rem] md:rounded-[2.5rem] border border-stone-200 shadow-sm hover:shadow-xl transition-all hover:border-amber-500">
                        <div className="w-14 h-14 md:w-16 md:h-16 bg-amber-50 rounded-2xl flex items-center justify-center mb-6 mx-auto group-hover:bg-amber-100">
                            <item.icon className="w-8 h-8 md:w-10 md:h-10 text-amber-700 group-hover:scale-110 transition-transform" />
                        </div>
                        <h3 className="text-lg md:text-xl font-bold font-serif text-stone-900 mb-2">{item.title}</h3>
                        <p className="text-stone-500 text-sm mb-6">{item.desc}</p>
                        <span className="text-amber-700 font-bold text-[9px] md:text-[10px] uppercase tracking-widest group-hover:underline flex items-center justify-center gap-2">Comenzar <Sparkles className="w-3 h-3" /></span>
                    </Link>
                ))}
            </div>
        </div>
    ...
    <section id="inicio" className="relative min-h-[70vh] md:min-h-[85vh] flex items-center justify-center overflow-hidden px-4">
        ...
        <div className="max-w-6xl mx-auto relative z-10 text-center">
            ...
            {!isAuthenticated ? (
                ...
                <div className="inline-flex items-center gap-3 px-6 md:px-10 py-4 md:py-5 border border-amber-500/20 bg-amber-500/5 backdrop-blur-xl rounded-full">
                    <div className="w-2.5 h-2.5 md:w-3 md:h-3 bg-green-500 rounded-full animate-pulse shadow-[0_0_15px_rgba(34,197,94,0.6)]"></div>
                    <p className="text-lg md:text-2xl font-serif italic text-amber-100">Sesión Activa. <span className="text-white">Explora lo nuevo hoy.</span></p>
                </div>
            )}
        </div>
    </section>
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#fdfcf8]">
            <main className="flex-grow">
                ...
                <div id="reseñas" className="py-16 md:py-24 max-w-7xl mx-auto px-4 md:px-6">
                    <div className="text-center mb-10 md:mb-16">
                        <p className="text-amber-800 font-bold tracking-[0.3em] text-[9px] md:text-[10px] mb-4 uppercase">Crónicas Recientes</p>
